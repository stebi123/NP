...
from app.schemas.staging import StagingCreate, StagingResponse, StagingQCUpdate, QCStatus
...
def create_staging_entry(
    ...
    new_staging = Staging(
        ...
        qc_status=staging_data.qc_status,        # automatically HOLD
        qc_done_on=staging_data.qc_done_on,      # automatically None
        approved_quantity=staging_data.approved_quantity,  # 0
        rejected_quantity=staging_data.rejected_quantity   # 0
    )
...
def filter_staging_entries(
    ...
    if date:
        ...
        query = query.filter(
            func.date(Staging.received_on) == date
        )
    else:
        if start_date:
